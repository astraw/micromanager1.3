...
public class TrackerControlDlg extends JDialog {
    ...
    * Create the dialog
    */
   public TrackerControlDlg(CMMCore core) {
      super();
      imageCounter_ = 0;
      limits_ = new MMRect();
      prefs_ = Preferences.userNodeForPackage(this.getClass());
      core_ = core;
      stage_ = core.getXYStageDevice();
      pixelSizeUm_ = core_.getPixelSizeUm();
      String camera = core_.getCameraDevice();
      try {
         mirrorX_ = core_.getProperty(camera, MMCoreJ.getG_Keyword_Transpose_MirrorX()).equals("1");
         mirrorY_ = core_.getProperty(camera, MMCoreJ.getG_Keyword_Transpose_MirrorY()).equals("1");
         rotate_ = core_.getProperty(camera, MMCoreJ.getG_Keyword_Transpose_SwapXY()).equals("1");
      ...
      final JButton topLeftButton = new JButton();
      topLeftButton.addActionListener(new ActionListener() {
         public void actionPerformed(final ActionEvent e) {
            double[] x = new double[1];
            double[] y = new double[1];
            try {
               core_.getXYPosition(stage_, x, y);
               limits_.xmin = x[0];
               limits_.ymin = y[0];
      ...
      topLeftButton.setBounds(10, 140, 106, 26);
      getContentPane().add(topLeftButton);

      final JButton bottomRightButton = new JButton();
      ...
      bottomRightButton.setBounds(10, 170, 106, 26);
      getContentPane().add(bottomRightButton);
      ...
      stackRadioButton_.setBounds(253, 185, 143, 24);
      ...
      rootLabel.setBounds(10, 265, 29, 16);
      ...
      clearButton.setBounds(10, 202, 106, 26);
   ...
   public void track() {
      ...
      if (image5dRadioButton_.isSelected()) {
         ...
         try {
            ...
            acq_.setImagePhysicalDimensions((int)core_.getImageWidth(), (int)core_.getImageHeight(), (int)core_.getBytesPerPixel());
   ...
   private boolean snapSingleImage() {
      ...
      try {
         ...
         if (acq_ != null) {
            ...
            Configuration cfg = core_.getSystemStateCache();
   ...
   private void processOneFrame(boolean moveStage) {
      ...
      if (moveStage) {
         ...
         try {
            ...
            core_.getXYPosition(stage_, xCur, yCur);
            if (acq_ != null) {
            ...
            if ((limits_.isValid() && limits_.isWithin(newX, newY)) || (!limits_.isValid())) {
               core_.setXYPosition(stage_, newX, newY);
               core_.waitForDevice(stage_);
               core_.getXYPosition(stage_, xCur, yCur);
