...
public:
    ...
    /**
     * Set the baud rate of the serial port.
     * Use an int value instead of the enum
     */
    void
    SetBaudRate( const int baudRate )
        throw( SerialPort::NotOpen,
               SerialPort::UnsupportedBaudRate,
               std::invalid_argument,
               std::runtime_error ) ;
    ...
    void
    SetNumOfStopBits( const int numOfStopBits )
        throw( SerialPort::NotOpen,
               std::invalid_argument )  ;
...
void
SerialPort::Open( const int      baudRate,
                  const CharacterSize charSize,
                  const Parity        parityType,
                  const StopBits      stopBits,
                  const FlowControl   flowControl )
...
void
SerialPort::SetBaudRate( const int baudRate )
    throw( UnsupportedBaudRate,
           NotOpen,
           std::invalid_argument )
{
...
void
SerialPort::SetCharSize( long int charSize )
    throw( NotOpen,
           std::invalid_argument )
{
    mSerialPortImpl->SetCharSize(charSize) ;
...
void
SerialPort::SetNumOfStopBits( const int numOfStopBits )
    throw( NotOpen,
           std::invalid_argument )
{
    mSerialPortImpl->SetNumOfStopBits(numOfStopBits) ;
           ...
           SerialPort::AlreadyOpen )
{
    ...
    /*
     * Now that the device is open, celar the O_NONBLOCK flag
    ...
    if ( fcntl( mFileDescriptor,
                ...
                0 ) < 0 )
    {
        printf ("Failed in fcntl asynchronous IO\n");
        throw SerialPort::OpenFailed( strerror(errno) ) ;
    }
     ...
     *
    int pid=getpid();
    int result=fcntl(mFileDescriptor,F_SETOWN,pid);
    int owner=fcntl(mFileDescriptor,F_GETOWN,0);
    if ( fcntl( mFileDescriptor,
                F_SETOWN,
                getpid() ) < 0 )
    {
       
       printf ("Failed in fcntl: %d %d %d %d %d %s\n",pid,mFileDescriptor,errno,owner,result,strerror(errno));
        throw SerialPort::OpenFailed( strerror(errno) ) ;
    }
    */
    ...
    cfsetspeed(&port_settings, B9600);
       ...
       port_settings.c_cflag |= CREAD | CLOCAL | CS8 | PARENB | CCTS_OFLOW | CRTS_IFLOW;
    #else
       port_settings.c_cflag |= CREAD | CLOCAL | CS8 | PARENB;
    #endif
    ...
    if ( tcsetattr( mFileDescriptor,
                    ...
                    &port_settings ) < 0 )
    {
       printf("Failed to write new settings to the port\n");
        throw SerialPort::OpenFailed( strerror(errno) ) ;
    }
...
void
SerialPort::SerialPortImpl::SetBaudRate( const int baudRate )
    throw( SerialPort::NotOpen,
           SerialPort::UnsupportedBaudRate,
           std::invalid_argument,
           std::runtime_error )
...
void
SerialPort::SerialPortImpl::SetCharSize( long int charSize )
    throw( SerialPort::NotOpen,
           std::invalid_argument,
           std::runtime_error )
{
   ...
   {
      case 5: 
         SetCharSize (CHAR_SIZE_5);
         break;
      case 6: 
         SetCharSize (CHAR_SIZE_6);
         break;
      case 7: 
         SetCharSize (CHAR_SIZE_7);
         break;
      case 8: 
...
void
SerialPort::SerialPortImpl::SetNumOfStopBits( const int numOfStopBits )
    throw( SerialPort::NotOpen,
           std::invalid_argument )
{
    switch( numOfStopBits )
           ...
           std::runtime_error )
{
    ...
    // Read a single byte into buffer buf_char
    while( 0 == read(mFileDescriptor,buf_char,1 ))
    {
           ...
           std::runtime_error )
{
    ...
    //
    // Write byte to the serial port, only when there is a byte
    //
    if (&dataByte > 0) {
       this->Write( &dataByte,
                    1 ) ;
