...
//
// CVS:           $Id: Andor.cpp 2183 2009-02-28 08:17:45Z nico $
...
int AndorCamera::Initialize()
{
  ...
  if(mb_canSetTemp) {
   ...
   if(!HasProperty(g_CycleTime))
   {
      ...
      nRet = CreateProperty(g_CycleTime, "0.0", MM::Float, true);
   }
   else
   {
      nRet = SetPropertyReadOnly(g_CycleTime, false);
      nRet = SetProperty(g_CycleTime, "0.0");
      nRet = SetPropertyReadOnly(g_CycleTime, true);
   }
   assert(nRet == DEVICE_OK);
...
long AndorCamera::GetReadoutTime()
{
   ...
   SetPropertyReadOnly(g_CycleTime, false);
   SetProperty(g_CycleTime, CDeviceUtils::ConvertToString(ActualInterval_ms_)); 
   SetPropertyReadOnly(g_CycleTime, true);
...
int AndorCamera::OnEMSwitch(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::AfterSet)
   {
      ...
      if (EMSwitch == "On") {
         ...
         ret = SetPropertyReadOnly(g_EMGainValue, false);
         UpdateEMGainRange();
         EMSwitch_ = true;
      } else {
         ret = SetEMCCDGain(0);
         ret = SetPropertyReadOnly(g_EMGainValue, true);
