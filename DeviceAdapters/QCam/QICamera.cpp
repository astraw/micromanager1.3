...
* Required by the MM::Camera API.
*/
int QICamera::StartSequenceAcquisition(long numImages, double intervalMs, bool stopOnOverflow)
{
#ifdef PRINT_FUNCTION_NAMES
   printf("QICamera::StartSequenceAcquisition\n");
#endif
...
* Logs error 
*/
int QICamera::ReportError(std::string message, int err)
{
   ostringstream os;
   os << message << err;
   LogMessage(os.str().c_str());
...
/**
* Helper function - sets software triggering mode
* and check it by capturing a frame 
*/
int QICamera::FinishSequenceMode()
{

#ifdef PRINT_FUNCTION_NAMES
   printf("QICamera::RestartSnapMode\n");
#endif

   LogMessage("Camera in softare trigger mode");
   return DEVICE_OK;
}


/**
* Helper function - waits for event signalled from the 
* preview and acquisition callbacks 
*/
int QICamera::WaitForFrameCaptured()
{
   unsigned long long		exposure;
...
#ifdef PRINT_FUNCTION_NAMES
   printf("QICamera::WaitForFrameCaptured\n");
#endif
...
/**
* Sends the captured immage to the Core
* and re-triggers the sequence capturing
* (unless HW triggering mode is set - to be implemented)
*/
...
#ifdef PRINT_FUNCTION_NAMES
   printf("QICamera::PushSequenceImage\n");
#endif
   // ToDo: check for time limits for ImageProcessor. 
   // Frames in the sequence can be lost if the processing takes longer than frame period

   // process image
   MM::ImageProcessor* ip = GetCoreCallback()->GetImageProcessor(this);      
   if (ip)                                                                   
   {                                                                         
      ret = ip->Process(m_snappedImageBuffer.GetPixelsRW(), GetImageWidth(), GetImageHeight(), GetImageBytesPerPixel());
      if (ret != DEVICE_OK)                                                  
         return; // Assume error is reported in ip->Process                                                        
   }                                                                         
