...
public class Z_Project implements PlugIn {
   ...
   Image5D doI5DProjection() {
      ...
      i5d.setCurrentPosition(0, 0, currentChannel-1, currentSlice-1, currentFrame-1);
      // Set overlay mode as in original
      resultI5D.show();
      resultI5D.setDisplayMode(i5d.getDisplayMode());

      // Copy Micro-Manager calibration
      Image5DWindow oldWindow = (Image5DWindow) i5d.getWindow();
      Image5DWindow newWindow = (Image5DWindow) resultI5D.getWindow();

      // TODO: metadata must be updated for the new image type ???
      AcquisitionData ad;
      try {
         ad = oldWindow.getAcquisitionData().createCopy();
         ad.setDimensions(ad.getNumberOfFrames(), ad.getNumberOfChannels(), 1); // ???
